...
export function TopupModal({ customer, onClose, onSaved }) {
  ...
  const handleSave = async () => {
    ...
    if (!form.topupStartDate) { alert("กรุณาระบุวันเริ่มชำระงวดแรก"); return; }
    setSaving(true);
    try {
      await createTopup({
        ...
        <div style={{ marginBottom: 16 }}>
          ...
          {schedule.length > 0 && (
            <div style={{ marginTop: 8, padding: "7px 12px", background: "rgba(45,212,191,.05)", border: "1px solid rgba(45,212,191,.15)", borderRadius: 8, fontSize: 12, display: "flex", gap: 16, flexWrap: "wrap" }}>
              <span style={{ color: BRAND.textSec }}>จำนวนงวด: <span style={{ color: BRAND.teal, fontWeight: 700 }}>{schedule.length} งวด</span></span>
              {form.interestAmount && (
